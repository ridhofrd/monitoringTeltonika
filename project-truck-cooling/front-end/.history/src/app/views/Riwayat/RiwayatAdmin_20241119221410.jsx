...
export default function RiwayatAdmin() {
  ...
  const handleCenterChange = () => {
    mapData.length
      ? mapData.slice(-1).map((data, index) => setCenter(data.log_latitude, data.log_longitude))
      : setCenter([-6.9175, 107.6191]);
  };
    ...
    fetch(
      ...
      .then((data) => {
        ...
        mapData.length
          ? mapData
              .slice(-1)
              .map((data, index) => console.log([data.log_latitude, data.log_longitude]))
          : setCenter([-6.9175, 107.6191]);
        console.log(center);
      })
      .catch((error) => {
    ...
    <Container>
      ...
      <Stack spacing={3}>
        ...
        >
          ...
          <MapContainer center={center} zoom={30} style={{ height: "100%", width: "100%" }}>
            <TileLayer
              url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
              attribution='&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
            />
